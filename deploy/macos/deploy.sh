...
header() {
    echo ""
    echo -e "${CYAN}========================================${NC}"
    echo -e "${CYAN}  Vulcan $VULCAN_VERSION — macOS 部署${NC}"
    echo -e "${CYAN}========================================${NC}"
    echo ""
}
...
do_install() {
    ...
    if [ "$DOCKER_MODE" = "docker" ]; then
        ...
        if command -v python3 >/dev/null 2>&1; then
            ...
            echo -e "${RED}[X] 请先安装 Python 3.11+: brew install python@3.11${NC}"
            echo -e "${GRAY}  或: https://python.org${NC}"
        fi
...
do_start() {
    ...
    if [ "$DOCKER_MODE" = "docker" ] && docker info >/dev/null 2>&1; then
        ...
        echo -e "   ${CYAN}Web UI:   http://localhost:3000${NC}"
        echo -e "   ${CYAN}API:      http://localhost:8000${NC}"
        echo -e "   日志: /tmp/vulcan-api.log, /tmp/vulcan-ui.log${NC}"
    fi
}
...
do_status() {
    ...
    if [ "$DOCKER_MODE" = "docker" ] && docker info >/dev/null 2>&1; then
        echo -e "${GRAY}模式: Docker${NC}"
        docker-compose ps
    else
        echo -e "${GRAY}模式: Native${NC}"
...
do_logs() {
    if [ "$DOCKER_MODE" = "docker" ] && docker info >/dev/null 2>&1; then
        ...
        echo "--- 后端日志 ---"
        tail -f /tmp/vulcan-api.log 2>/dev/null || echo "(无日志)"
    fi
}
...
show_help() {
    ...
    echo -e "  Docker 模式:    Docker Desktop for macOS"
